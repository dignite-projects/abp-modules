---
name: notifications-invariants
description: Hard invariants of the Dignite.Abp.Notifications module — stable [NotificationDataType] discriminators with System.Text.Json only and no polymorphic ETO members, DI lifetime discipline, the Notifier plugin boundary against Abstractions, best-effort single-recipient cancellation-aware delivery, both operation modes, entity-scoped subscription identity, definition requirements re-applied at delivery, bounded recipient batching and keyset paging, and the anti-scope list of everything four subtraction rounds removed. Read BEFORE touching NotificationData, any Notifier, the distributor, or a service's DI lifetime under notifications/.
---

# Hard Invariants — Read Before Touching Notifications / Notifiers / DI Lifetimes

> Module orientation (layer map, the plugin boundary, the two operation modes, "add a feature") lives in
> [`notifications/CLAUDE.md`](../../../CLAUDE.md), which loads automatically. This file exists because this
> repo is a from-scratch rewrite of a legacy implementation that shipped with the exact bugs described below —
> these are not style preferences, they're the reasons this repo exists. Do not reintroduce them.
>
> For this module's *conventions* (aggregate shapes, `INotificationStore`, hand-written mapping, explicit
> controllers, the permission-checker seam) see the `notifications-conventions` skill. Generic ABP conventions
> live in the repo-root `abp-*` skills.
