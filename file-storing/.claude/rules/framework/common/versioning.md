...
from a single `<Version>` in the **repository root** `Directory.Build.props` — all 10 packable projects here,
all 15 in `notifications/`, and both Angular packages. There is no per-module and no per-project version.

A consequence you will see in the history: a change to `notifications/` alone still bumps the version of
every package here, with no content change. **That is expected, not a mistake** — don't "fix" it, and don't
...
must stay **≥ 10** permanently: the legacy `Dignite.FileExplorer.*` package line is published on NuGet up to
`3.8.2` under these same PackageIds, and MAJOR ≥ 10 is what keeps these releases winning "latest version"
resolution.
...
- **Keep the Angular package in step.** `angular/projects/file-explorer/package.json`'s version must equal
  `<Version>`; CI and the release workflow run `.github/scripts/verify-version-lockstep.ps1`, which fails the
  build when either module's Angular package drifts from the .NET version.
- `<AssemblyVersion>` is pinned separately (`1.0.0.0`) and is **not** bumped in lockstep with `<Version>` —
  keeping it stable avoids strong-name/binding churn for consumers across releases. Nothing in *this* module
  serializes CLR type identities, but the sibling `notifications/` module does and depends on this pin, so it
  is repo-wide and not negotiable here.
