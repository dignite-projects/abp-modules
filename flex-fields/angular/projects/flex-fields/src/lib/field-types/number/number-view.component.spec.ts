import { TestBed } from '@angular/core/testing';
import { NumberViewComponent } from './number-view.component';

describe('NumberViewComponent', () => {
  function render(value: unknown, showInList = false) {
    const fixture = TestBed.createComponent(NumberViewComponent);
    fixture.componentRef.setInput('value', value);
    fixture.componentRef.setInput('showInList', showInList);
    fixture.detectChanges();
    return fixture;
  }

  it('renders the raw value bare in list mode', () => {
    expect(render(42, true).nativeElement.textContent.trim()).toBe('42');
  });

  it('renders the raw value inside the label wrapper otherwise', () => {
    expect(render(42, false).nativeElement.textContent).toContain('42');
  });
});
