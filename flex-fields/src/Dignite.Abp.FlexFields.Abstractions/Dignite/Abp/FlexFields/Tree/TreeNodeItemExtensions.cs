...
public static class TreeNodeItemExtensions
{
    ...
    public static bool ContainsValue(this IEnumerable<TreeNodeItem> nodes, string value)
    {
    ...
    /// <param name="value">要查找的值。</param>
    /// <returns>如果存在则返回 true，否则返回 false。</returns>
    public static bool ContainsValue(this TreeNodeItem node, string value)
    {
    ...
    public static TreeNodeItem? Find(this IEnumerable<TreeNodeItem> nodes, string value)
    {
        foreach (var node in nodes)
        {
            if (node.Value == value)
